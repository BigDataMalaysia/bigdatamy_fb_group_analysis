...
class Group(object):
    ...
    def fetch(self, oath_access_token, max_pages=None):
        """
        ...
        for post in raw_post_data:
            ...
            #comments = list(graph.get('/v2.6/{}/comments'.format(post_obj.fb_id), page=True))
            #logging.debug("comments: %d, %s", len(comments), pprint.pformat(comments))
